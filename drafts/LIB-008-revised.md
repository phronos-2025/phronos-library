...
How can cognitive instruments be designed to be engaging, valid, and interpretable without causing harm?
...
1. **Gamification validity** (H8.1) — Can game-based formats maintain psychometric properties?
2. **Task parameter calibration** — How do design choices (clue count, time pressure, constraints) affect what we measure?
3. **Harm prevention** — How do we present results without causing psychological harm?
...
**Evidence:**
- [MTH-002.1](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/) documents optimal clue counts:
...
- Constraint effects are substantial: Cohen's d = 1.02 between unconstrained DAT and constrained INS-001.2 ([MTH-002.1 §4.6](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/#4-dat-calibration))
- Relevance-spread redundancy required metric redesign: original relevance metric showed r = -0.77 with spread in INS-001.1, r = -0.61 in INS-001.2 ([MTH-002.1 §2.4](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/#2-metric-overview))
...
|--------|--------------|---------|
| **Pedersen et al. 2023** | Skill Lab validation | Supports H8.1 for memory and attention games. Creativity/divergent thinking games not validated. See [MTH-002.1](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/) for INS-001-specific calibration. |
...
| **MTH-002.1** | Spread-fidelity calibration | Documents metric independence problem and solution; optimal clue counts; constraint effects. [Full document](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/) |
...
| **Constraint calibration vs. validation** | Documented but not validated. MTH-002.1 calibrates INS-001 against DAT; whether constrained spread measures the same construct is assumed, not tested. | [MTH-002.1 §4.6](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/#4-dat-calibration) |
...
[MTH-002.1](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/) documents this tension empirically:
...
Creativity-specific gamification is not validated—INS-001 design choices are empirically calibrated ([MTH-002.1](https://phronos.org/methods/semantic-association-metrics/spread-fidelity-scoring/)) but lack external criterion validation against established creativity measures. We know how our scores relate to DAT; we don't know if the game format preserves what DAT measures.
